 ...
 *
 * Nothing runtime-instantiable exports from here — contract **types** (e.g.
 * `OutboxRepo`) are re-exported for consumers like `@cre/workers`. Import
 * `buildApp` (or the individual modules) for tests and embedding.
 */
// Loads `packages/api/.env` (cwd-relative) before anything reads configuration.
// dotenv never overwrites variables that are already set, so values injected by
// docker compose / CI / the shell always win over the file.
...
export type { OutboxRepo } from './ports';
